...
- (BOOL)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"setup" isEqualToString:method]) {
    ...
    [self displayIncomingCall:argsMap[@"uuid"]
                       handle:argsMap[@"handle"]
                   handleType:argsMap[@"handleType"]
                     hasVideo:[argsMap[@"hasVideo"] boolValue]
                   callerName:argsMap[@"callerName"]
                      payload:argsMap[@"additionalData"]];
    result(nil);
  } else if ([@ "startCall" isEqualToString:method]) {
    [self startCall:argsMap[@"uuid"]
             handle:argsMap[@"handle"]
         callerName:argsMap[@"callerName"]
         handleType:argsMap[@"handleType"]
              video:[argsMap[@"hasVideo"] boolValue]];
    result(nil);
    ...
    [self reportEndCallWithUUID:argsMap[@"uuid"]
                         reason:[argsMap[@"reason"] intValue]];
    result(nil);
    ...
    [self updateDisplay:argsMap[@"uuid"]
             callerName:argsMap[@"callerName"]
                    uri:argsMap[@"handle"]];
    result(nil);
    ...
    [self reportUpdatedCall:argsMap[@"uuid"]
          contactIdentifier:argsMap[@"callerName"]];
    result(nil);
  } else {
    return NO;
  }
...
- (void)sendEventWithNameWrapper:(NSString *)name body:(NSDictionary *)body {
  ...
  if (_delegate && [_delegate respondsToSelector:@selector(onCallEvent:
                                                          withCallData:)]) {
...
+ (void)initCallKitProvider {
  if (settings == nil) {
    settings = [[NSUserDefaults standardUserDefaults]
        dictionaryForKey:@"CallKeepSettings"];
  }
  if (sharedProvider == nil) {
    sharedProvider = [[CXProvider alloc]
        initWithConfiguration:[CallKeep getProviderConfiguration:settings]];
  }
}
...
- (void)pushRegistry:(PKPushRegistry *)registry
  ...
  [self sendEventWithNameWrapper:CallKeepPushKitToken
                            body:@{@"token" : hexToken}];
}

- (NSString *)createUUID {
  CFUUIDRef uuidObject = CFUUIDCreate(kCFAllocatorDefault);
  NSString *uuidStr = (NSString *)CFBridgingRelease(
...
- (void)displayIncomingCall:(NSString *)uuidString
  ...
  [CallKeep reportNewIncomingCall:uuidString
                           handle:handle
...
- (void)startCall:(NSString *)uuidString
  ...
  CXHandle *callHandle = [[CXHandle alloc] initWithType:_handleType
                                                  value:handle];
  ...
  CXTransaction *transaction =
      [[CXTransaction alloc] initWithAction:startCallAction];
  [self requestTransaction:transaction
       withSuccessListener:^(CXAction *action) {
         // CXStartCallAction
         if ([action isKindOfClass:[CXStartCallAction class]]) {
           ...
           [sharedProvider reportCallWithUUID:startCallAction.callUUID
                                      updated:callUpdate];
         }
       }];
}
...
- (void)answerIncomingCall:(NSString *)uuidString {
  ...
  CXTransaction *transaction =
      [[CXTransaction alloc] initWithAction:answerCallAction];
...
- (void)endCall:(NSString *)uuidString {
  ...
  CXTransaction *transaction =
      [[CXTransaction alloc] initWithAction:endCallAction];
...
- (void)endAllCalls {
#ifdef DEBUG
  NSLog(@"[CallKeep][endAllCalls] calls = %@",
        self.callKeepCallController.callObserver.calls);
#endif
  for (CXCall *call in self.callKeepCallController.callObserver.calls) {
    CXEndCallAction *endCallAction =
        [[CXEndCallAction alloc] initWithCallUUID:call.UUID];
    CXTransaction *transaction =
        [[CXTransaction alloc] initWithAction:endCallAction];
    [self requestTransaction:transaction];
  }
...
#ifdef DEBUG
  NSLog(@"[CallKeep][setOnHold] uuidString = %@, shouldHold = %d", uuidString,
        shouldHold);
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
  CXSetHeldCallAction *setHeldCallAction =
      [[CXSetHeldCallAction alloc] initWithCallUUID:uuid onHold:shouldHold];
...
- (void)reportConnectingOutgoingCallWithUUID:(NSString *)uuidString {
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
  [self.callKeepProvider reportOutgoingCallWithUUID:uuid
...
- (void)updateDisplay:(NSString *)uuidString
           callerName:(NSString *)callerName
                  uri:(NSString *)uri {
#ifdef DEBUG
  NSLog(@"[CallKeep][updateDisplay] uuidString = %@ displayName = %@ uri = %@",
        uuidString, callerName, uri);
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
...
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
  CXSetMutedCallAction *setMutedAction =
...
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
  CXPlayDTMFCallAction *dtmfAction = [[CXPlayDTMFCallAction alloc]
      initWithCallUUID:uuid
                digits:key
                  type:CXPlayDTMFCallActionTypeHardPause];
  CXTransaction *transaction = [[CXTransaction alloc] init];
  ...
  [self.callKeepCallController
      requestTransaction:transaction
              completion:^(NSError *_Nullable error) {
                if (error != nil) {
                  NSLog(@"[CallKeep][requestTransaction] Error requesting "
                        @"transaction (%@): (%@)",
                        transaction.actions, error);
                } else {
                  NSLog(@"[CallKeep][requestTransaction] Requested transaction "
                        @"successfully");
                  if (onSuccess) {
...
+ (BOOL)isCallActive:(NSString *)uuidString {
  ...
  for (CXCall *call in callObserver.calls) {
    NSLog(@"[CallKeep] isCallActive %@ %d ?", call.UUID,
...
+ (void)endCallWithUUID:(NSString *)uuidString reason:(int)reason {
#ifdef DEBUG
  NSLog(@"[CallKeep][reportEndCallWithUUID] uuidString = %@ reason = %d",
        uuidString, reason);
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
...
+ (void)reportNewIncomingCall:(NSString *)uuidString
  ...
  CXCallUpdate *callUpdate = [CallKeep createCallUpdate];
  callUpdate.remoteHandle = [[CXHandle alloc] initWithType:_handleType
                                                     value:handle];
  ...
  [sharedProvider
      reportNewIncomingCallWithUUID:uuid
                         ...
                         completion:^(NSError *_Nullable error) {
                           ...
                           if (error == nil) {
                             // Workaround per
                             // https://forums.developer.apple.com/message/169511
...
+ (CXCallUpdate *)createCallUpdate {
  CXCallUpdate *callUpdate = [[CXCallUpdate alloc] init];
  callUpdate.supportsDTMF =
      settings[@"supportsDTMF"] ? [settings[@"supportsDTMF"] boolValue] : NO;
  callUpdate.supportsHolding = settings[@"supportsHolding"]
                                   ? [settings[@"supportsHolding"] boolValue]
                                   : NO;
  callUpdate.supportsGrouping = settings[@"supportsGrouping"]
                                    ? [settings[@"supportsGrouping"] boolValue]
                                    : NO;
  callUpdate.supportsUngrouping =
      settings[@"supportsUngrouping"]
          ? [settings[@"supportsUngrouping"] boolValue]
          : NO;
...
// @deprecated
- (void)reportUpdatedCall:(NSString *)uuidString
        contactIdentifier:(NSString *)contactIdentifier {
#ifdef DEBUG
  NSLog(@"[CallKeep][reportUpdatedCall] contactIdentifier = %@",
        contactIdentifier);
#endif
  NSUUID *uuid = [[NSUUID alloc] initWithUUIDString:uuidString];
  CXCallUpdate *callUpdate = [[CXCallUpdate alloc] init];
...
+ (CXProviderConfiguration *)getProviderConfiguration:(NSDictionary *)settings {
  ...
  CXProviderConfiguration *providerConfiguration =
      [[CXProviderConfiguration alloc] initWithLocalizedName:appName];
  ...
  if (settings[@"handleType"]) {
    ...
    providerConfiguration.supportedHandleTypes =
        [NSSet setWithObjects:[NSNumber numberWithInteger:_handleType], nil];
  } else {
  ...
  if (settings[@"supportsVideo"]) {
    providerConfiguration.supportsVideo =
        [settings[@"supportsVideo"] boolValue];
  }
  if (settings[@"maximumCallGroups"]) {
    providerConfiguration.maximumCallGroups =
        [settings[@"maximumCallGroups"] integerValue];
  }
  if (settings[@"maximumCallsPerCallGroup"]) {
    providerConfiguration.maximumCallsPerCallGroup =
        [settings[@"maximumCallsPerCallGroup"] integerValue];
  }
  ...
  if (@available(iOS 11.0, *)) {
    if (settings[@"includesCallsInRecents"]) {
      providerConfiguration.includesCallsInRecents =
          [settings[@"includesCallsInRecents"] boolValue];
    }
  }
...
+ (BOOL)application:(UIApplication *)application
  ...
  /*
   ...
   postNotificationName:CallKeepHandleStartCallNotification object:self
   userInfo:userInfo];
   return YES;
   }
   return NO;
...
+ (BOOL)application:(UIApplication *)application
    continueUserActivity:(NSUserActivity *)userActivity
      restorationHandler:
          (void (^)(NSArray<id<UIUserActivityRestoring>>
                        *_Nonnull restorableObjects))restorationHandler {
#ifdef DEBUG
  ...
  if (@available(iOS 13, *)) {
    ...
    // callCapability is not available on iOS > 13.2, but it is in 13.1
    // weirdly...
    if ([intent respondsToSelector:@selector(callCapability)]) {
      ...
      isAudioCall = [userActivity.activityType
          isEqualToString:INStartAudioCallIntentIdentifier];
      isVideoCall = [userActivity.activityType
          isEqualToString:INStartVideoCallIntentIdentifier];
    }
  } else {
#endif
    // XCode 10 and below
  ...
  if (isAudioCall) {
    ...
    INStartVideoCallIntent *startVideoCallIntent =
        (INStartVideoCallIntent *)interaction.intent;
  ...
  if (handle != nil && handle.length > 0) {
    NSDictionary *userInfo =
        @{@"handle" : handle, @"name" : displayName, @"video" : @(isVideoCall)};

    CallKeep *callKeep = [CallKeep allocWithZone:nil];
    [callKeep sendEventWithNameWrapper:CallKeepDidReceiveStartCallAction
                                  body:userInfo];
    return YES;
  }
  return NO;
...
- (void)provider:(CXProvider *)provider
  ...
  [self sendEventWithNameWrapper:CallKeepDidReceiveStartCallAction
                            body:@{
                              @"callUUID" :
                                  [action.callUUID.UUIDString lowercaseString],
...
#endif
  [self configureAudioSession];
  [self sendEventWithNameWrapper:CallKeepPerformAnswerCallAction
                            body:@{
                              @"callUUID" :
                                  [action.callUUID.UUIDString lowercaseString]
                            }];
...
#endif
  [self sendEventWithNameWrapper:CallKeepPerformEndCallAction
                            body:@{
                              @"callUUID" :
                                  [action.callUUID.UUIDString lowercaseString]
                            }];
...
- (void)provider:(CXProvider *)provider
  ...
  [self sendEventWithNameWrapper:CallKeepDidToggleHoldAction
                            body:@{
                              @"hold" : @(action.onHold),
                              @"callUUID" :
                                  [action.callUUID.UUIDString lowercaseString]
...
#endif
  [self sendEventWithNameWrapper:CallKeepPerformPlayDTMFCallAction
                            body:@{
...
- (void)provider:(CXProvider *)provider
  ...
  [self sendEventWithNameWrapper:CallKeepDidPerformSetMutedCallAction
                            body:@{
                              @"muted" : @(action.muted),
                              @"callUUID" :
                                  [action.callUUID.UUIDString lowercaseString]
...
- (void)provider:(CXProvider *)provider
  ...
  if ([action isKindOfClass:[CXAnswerCallAction class]]) {
    ...
    body = @{
      @"callUUID" : [answerAction.callUUID.UUIDString lowercaseString],
      @"action" : CallKeepActionAnswer
    };
  } else if ([action isKindOfClass:[CXEndCallAction class]]) {
    CXEndCallAction *answerAction = ((CXEndCallAction *)action);
    body = @{
      @"callUUID" : [answerAction.callUUID.UUIDString lowercaseString],
      @"action" : CallKeepActionEnd
    };
  }
